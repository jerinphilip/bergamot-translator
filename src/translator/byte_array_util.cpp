...
// This struct and the getter are copied from the marian source, because it's
// located inside src/common/binary.cpp:15 and we can't include it.
struct Header {
  uint64_t nameLength;
  uint64_t type;
...
bool validateBinaryModel(const AlignedMemory& model, uint64_t fileSize) {
  ...
  uint64_t memoryNeeded =
      sizeof(uint64_t) * 2;  // We keep track of how much memory we would need
                             // if we have a complete file
  uint64_t numHeaders;
  if (fileSize >=
      memoryNeeded) {  // We have enough filesize to fetch the headers.
  ...
  // Calculate how many bytes we are going to for reading just the names and the
  // shape
  for (uint64_t i = 0; i < numHeaders; i++) {
    memoryNeeded +=
        headers[i].nameLength + headers[i].shapeLength * sizeof(int);
    // Advance the pointers.
    get<char>(current, headers[i].nameLength);
    get<int>(current, headers[i].shapeLength);
  }

  // Before we start reading the data, there is a small padding to ensure
  // alignment Read that in, before calculating the actual tensor memory
  // requirements.
  uint64_t aligned_offset;
  if (fileSize >= memoryNeeded) {
    aligned_offset =
        *get<uint64_t>(current);  // Offset to align memory to 256 size
  ...
  // If this final check passes, the file is at least big enough to contain the
  // model
  if (fileSize >= memoryNeeded) {
    return true;
  } else {
...
AlignedMemory getModelMemoryFromConfig(marian::Ptr<marian::Options> options) {
  ...
  ABORT_IF(models.size() != 1,
           "Loading multiple binary models is not supported for now as it is "
           "not necessary.");
  marian::filesystem::Path modelPath(models[0]);
  ABORT_IF(modelPath.extension() != marian::filesystem::Path(".bin"),
           "The file of binary model should end with .bin");
  AlignedMemory alignedMemory = loadFileToMemory(models[0], 256);
  return alignedMemory;
}

AlignedMemory getShortlistMemoryFromConfig(
...
void getVocabsMemoryFromConfig(
    marian::Ptr<marian::Options> options,
  ...
  for (size_t i = 0; i < vfiles.size(); ++i) {
    auto m = vocabMap.emplace(
        std::make_pair(vfiles[i], std::shared_ptr<AlignedMemory>()));
    if (m.second) {
      m.first->second =
          std::make_shared<AlignedMemory>(loadFileToMemory(vfiles[i], 64));
    }
