...
class Service {
 public:
  /// Construct Service from Marian options. If memoryBundle is empty, Service
  /// is initialized from file-based loading. Otherwise, Service is initialized
  /// from the given bytearray memories.
  ...
  /// Construct Service from a string configuration. If memoryBundle is empty,
  /// Service is initialized from file-based loading. Otherwise, Service is
  /// initialized from the given bytearray memories.
  /// @param [in] config string parsable as YAML expected to adhere with marian
  /// config
  /// @param [in] memoryBundle holds all byte-array memories. Can be a
  /// set/subset of model, shortlist, vocabs and ssplitPrefixFile bytes.
  /// Optional.
  ...
  std::vector<Response> translateMultiple(
      std::vector<std::string> &&source, TranslationRequest translationRequest);
