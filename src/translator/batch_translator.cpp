...
void BatchTranslator::initialize() {
  ...
  bool check = options_->get<bool>("check-bytearray",
                                   false);  // Flag holds whether validate the
                                            // bytearray (model and shortlist)
  ...
  if (modelMemory_->size() > 0 &&
      modelMemory_->begin() !=
          nullptr) {  // If we have provided a byte array that contains the
