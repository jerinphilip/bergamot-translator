...
struct MemoryBundle {
  ...
  /// If two vocabularies are the same (based on the filenames), two entries
  /// (shared pointers) will be generated which share the same AlignedMemory
  /// object.
...
// @TODO at the moment the usage of string_view in this repository is a hot mess
// and a disaster waiting to happen. ssplit uses std::string_view if the
// compiler supports c++17, else falls back to c++11 and absl::string_view
// bergamot-translator uses, depending on the source file std::string_view
// (which will break if ssplit-cpp uses absl::string_view) and
// marian::string_view which is an export of (confusingly) the sentencepiece
// module that marian has. marian::string_view is our addition to the marian
// fork, which will make merging even nicer. Not. This is just an ugly patchwork
// that allos gcc5, our lowest targetted gcc to run. We don't actually try to
// run on older compilers.
