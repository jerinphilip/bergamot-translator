...
// constructor of the TranslationModel class still expects 2 entries in this
// case, where each entry has the shared ownership of the same AlignedMemory
// object. This function prepares these smart pointer based AlignedMemory
// objects for unique AlignedMemory objects passed from JS.
std::vector<std::shared_ptr<AlignedMemory>> prepareVocabsSmartMemories(
    std::vector<AlignedMemory*>& vocabsMemories) {
  auto sourceVocabMemory =
      std::make_shared<AlignedMemory>(std::move(*(vocabsMemories[0])));
  ...
  if (vocabsMemories.size() == 2) {
    auto targetVocabMemory =
        std::make_shared<AlignedMemory>(std::move(*(vocabsMemories[1])));
...
marian::bergamot::MemoryBundle prepareMemoryBundle(
    AlignedMemory* modelMemory, AlignedMemory* shortlistMemory,
...
TranslationModel* TranslationModelFactory(
    const std::string& config, AlignedMemory* modelMemory,
    AlignedMemory* shortlistMemory,
    std::vector<AlignedMemory*> uniqueVocabsMemories) {
  return new TranslationModel(
      config, std::move(prepareMemoryBundle(modelMemory, shortlistMemory,
...
EMSCRIPTEN_BINDINGS(translation_model) {
  ...
  // ^ We redirect Service::translateMultiple to WASMBound::translate instead.
  // Sane API is translate. If and when async comes, we can be done with this
  // inconsistency.
